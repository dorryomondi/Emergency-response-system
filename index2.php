...
$pid = $_SESSION['patname'];
$sql = "SELECT `Emergency_ID`, `Patient_ID`, `Hospital_ID`, `Medical_Services`, `Prior_Medical_Condition`, `Blood_Type`, `Time`, `Location`, `Status` FROM `emergency_response` WHERE `Patient_ID` = '$pid'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
